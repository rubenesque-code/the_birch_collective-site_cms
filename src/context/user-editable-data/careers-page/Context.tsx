...
type ContextValue = { store: Store } & {
  revision: {
    isChange: boolean;
    undoKey: string;
    saveData: Partial<MyDb["pages"]["careers"]> | null;
    handleUndo: () => void;
    onSaveSuccess: () => void;
  };
};
...
function useThisContext() {
  ...
  if (!context)
    throw new Error("Missing CareersPageDataCx.Provider in the tree");

  return context;
}

function CareersPageDataCx() {
  throw new Error(
    "CareersPageDataCx exists for naming purposes only and should not be used as a component",
  );
}

export { CareersPageDataCx };

CareersPageDataCx.Provider = Provider;
CareersPageDataCx.use = useThisContext;
