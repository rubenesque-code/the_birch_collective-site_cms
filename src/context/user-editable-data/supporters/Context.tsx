...
function useContext() {
  ...
  if (!context)
    throw new Error("Missing SupportersDataCx.Provider in the tree");

  return context;
}

function SupportersDataCx() {
  throw new Error(
    "SupportersDataCx exists for naming purposes only and should not be used as a component",
  );
}

export { SupportersDataCx };

SupportersDataCx.Provider = Provider;
SupportersDataCx.use = useContext;
