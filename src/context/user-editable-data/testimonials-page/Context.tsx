...
type ContextValue = { store: Store } & {
  revision: {
    isChange: boolean;
    undoKey: string;
    saveData: Partial<MyDb["pages"]["testimonials"]>;
    handleUndo: () => void;
    onSaveSuccess: () => void;
  };
};
...
function Provider({
  ...
  initData: MyDb["pages"]["testimonials"];
}) {
  ...
  const value: ContextValue = {
    ...
    revision: {
      isChange,
      undoKey,
      saveData: { id: "testimonials-page", ...saveData },
      handleUndo,
      onSaveSuccess: () => {
...
function useThisContext() {
  ...
  if (!context)
    throw new Error("Missing TestimonialsPageDataCx.Provider in the tree");

  return context;
}

function TestimonialsPageDataCx() {
  throw new Error(
    "TestimonialsPageDataCx exists for naming purposes only and should not be used as a component",
  );
}

export { TestimonialsPageDataCx };

TestimonialsPageDataCx.Provider = Provider;
TestimonialsPageDataCx.use = useThisContext;
