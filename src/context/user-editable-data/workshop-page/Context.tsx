...
function useThisContext() {
  ...
  if (!context)
    throw new Error("Missing WorkshopPageDataCx.Provider in the tree");

  return context;
}

function WorkshopPageDataCx() {
  throw new Error(
    "WorkshopPageDataCx exists for naming purposes only and should not be used as a component",
  );
}

export { WorkshopPageDataCx };

WorkshopPageDataCx.Provider = Provider;
WorkshopPageDataCx.use = useThisContext;
