...
type ContextValue = { store: Store } & {
  revision: {
    isChange: boolean;
    undoKey: string;
    saveData: Partial<MyDb["pages"]["workshops"]>;
    handleUndo: () => void;
    onSaveSuccess: () => void;
  };
};
...
function Provider({
  ...
  const value: ContextValue = {
    ...
    revision: {
      isChange,
      undoKey,
      saveData: { id: "workshops-page", ...saveData },
      handleUndo,
      onSaveSuccess: () => {
...
function useThisContext() {
  ...
  if (!context)
    throw new Error("Missing WorkshopsPageDataCx.Provider in the tree");

  return context;
}

function WorkshopsPageDataCx() {
  throw new Error(
    "WorkshopsPageDataCx exists for naming purposes only and should not be used as a component",
  );
}

export { WorkshopsPageDataCx };

WorkshopsPageDataCx.Provider = Provider;
WorkshopsPageDataCx.use = useThisContext;
